...
const bookAppointment = async (
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    return { appointmentData, patientData, paymentUrl: session.url };
  });

  return {
    appointmentData: result.appointmentData,
    payment: result.patientData,
...
const initiatePayment = async (paymentIntentId: string, user: IRequestUser) => {
  const patientData = await prisma.patient.findUniqueOrThrow({
    where: {
      email: user.email,
    },
