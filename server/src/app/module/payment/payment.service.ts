...
const handleStripeWebhookEvent = async (event: Stripe.Event) => {
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      console.log(
        `Payment checkout.session.completed for appointment ${appointmentId} and payment ${paymentId}`,
      );
      break;
    }
    case "checkout.session.expired": {
      const session = event.data.object;
      console.log(
        `Checkout session ${session.id} expired. Marking associated payment as expired`,
      );
      break;
    }
